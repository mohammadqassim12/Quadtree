...
int main() {
  ...
  printf("\n\n");
    // Test #9.1 - split_tree() - Part 1: Nothing above threshold
        Image *im_1 = readPGMimage("test.pgm");
        Quad * root_2 = NULL;
        sx = im_1->sx;
        root_2 = new_Quad(0,0,512,512,0,512);
        printf("*** NOTE: Depth does not matter here, it matters that the values appear in one of the nodes\n");
        printf("-------------------------------------------------------------------------------------------");
        printf("\n*** Original Quad\n");
        BST_postorder(root_2, 0);
        printf("*** Expected Result:\n");
        printf("Depth=0, key=0, tx:ty (0:0), w=512, h=512, wsplit=0\n");
        root_2 = split_tree(im_1,root_2,256);
        printf("\n*** Results of split_tree() - Does not split\n");
        BST_postorder(root_2, 0);
        printf("-------------------------------------------------------------------------------------------\n");
        
      // Test #9.3 - split_tree() - Part 2: Root node above threshold
        printf("-------------------------------------------------------------------------------------------");
        printf("\n*** Original Quad\n");
        BST_postorder(root_2, 0);
        printf("*** Expected Result:\n");
        printf("Depth=0, key=0, tx:ty (0:0), w=512, h=256, wsplit=1\n\
    Depth=1, key=131072, tx:ty (0:256), w=512, h=256, wsplit=1\n");
        root_2 = split_tree(im_1,root_2,0);
        printf("\n*** Result of split_tree() - Splits root\n");
        BST_postorder(root_2, 0);
        printf("-------------------------------------------------------------------------------------------\n");
        
      // Test #9.5 - split_tree() - Part 3: Only part of the BST splits
        printf("-------------------------------------------------------------------------------------------");
        printf("\n*** Original Quad\n");
        BST_postorder(root_2, 0);
        printf("*** Expected Result:\n");
        printf("Depth=2, key=256, tx:ty (256:0), w=256, h=256, wsplit=0\n\
    Depth=1, key=0, tx:ty (0:0), w=256, h=256, wsplit=0\n\
    Depth=0, key=131072, tx:ty (0:256), w=512, h=256, wsplit=1\n");
        root_2 = split_tree(im_1,root_2,154);
        printf("\n*** Result of split_tree() - Only one splits\n");
        BST_postorder(root_2, 0);
        printf("-------------------------------------------------------------------------------------------\n");
        
      // Test #9.7 - split_tree() - Part 4: Split entire BST
        printf("-------------------------------------------------------------------------------------------");
        printf("\n*** Original Quad\n");
        BST_postorder(root_2, 0);
        printf("*** Expected Result:\n");
        printf("Depth=1, key=0, tx:ty (0:0), w=256, h=128, wsplit=1\n\
    Depth=2, key=65536, tx:ty (0:128), w=256, h=128, wsplit=1\n\
    Depth=3, key=131328, tx:ty (256:256), w=256, h=256, wsplit=0\n\
    Depth=2, key=131072, tx:ty (0:256), w=256, h=256, wsplit=0\n\
    Depth=1, key=65792, tx:ty (256:128), w=256, h=128, wsplit=1\n\
    Depth=0, key=256, tx:ty (256:0), w=256, h=128, wsplit=1\n");
        root_2 = split_tree(im_1,root_2,0);
        printf("\n*** Result of split_tree() - Split entire BST\n");
        BST_postorder(root_2, 0);
        printf("-------------------------------------------------------------------------------------------\n");
        root_2 = delete_BST(root_2);
        root_2 = new_Quad(1,1,101,101,0,512);
      // Test #9.9 - split_tree() - Part 5: Split odd
        printf("-------------------------------------------------------------------------------------------");
        printf("\n*** Original Quad\n");
        BST_postorder(root_2, 0);
        printf("*** Expected Result:\n");
        printf("Depth=1, key=26113, tx:ty (1:51), w=101, h=50, wsplit=1\n\
    Depth=0, key=513, tx:ty (1:1), w=101, h=50, wsplit=1\n");
        root_2 = split_tree(im_1,root_2,0);
        printf("\n*** Result of split_tree() - Split odd\n");
        BST_postorder(root_2, 0);
        printf("-------------------------------------------------------------------------------------------\n");
  ...
  if (get_colour(im, new) != 198) {
    ...
    // Test #10.1 - similar() - Part 1: Above by significant
        if(similar(im, new, 170)==1)
        {
            printf("Passed test #10.1\n");
        }
        else
        {
            printf("Failed test #10.1 - similar() did not return correct result.\n");
        }

        // Test #10.3 - similar() - Part 2: Above by barely
        if(similar(im, new, 163)==1)
        {
            printf("Passed test #10.3\n");
        }
        else
        {
            printf("Failed test #10.3 - similar() did not return correct result.\n");
        }

        // Test #10.5 - similar() - Part 3: Equal
        if(similar(im, new, 162)==1)
        {
            printf("Passed test #10.5\n");
        }
        else
        {
            printf("Failed test #10.5 - similar() did not return correct result.\n");
        }
    // Test #10.7 - similar() - Part 4: Below by significant
        if(similar(im, new, 161)==0)
        {
            printf("Passed test #10.7\n");
        }
        else
        {
            printf("Failed test #10.7 - similar() did not return correct result.\n");
        }

        // Test #10.9 - similar() - Part 5: Below by significant
        if(similar(im, new, 1)==0)
        {
            printf("Passed test #10.9\n");
        }
        else
        {
            printf("Failed test #10.9 - similar() did not return correct result.\n");
        }
